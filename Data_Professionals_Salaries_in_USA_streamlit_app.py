...
data=data.rename(columns={'company_txt':'Company Name', 'Age':'Company Age', "google_an":"google analytics", "Location":"City", "Job Location": "State Code", "Avg Salary(K)":"Average Salary (K USD)", "Lower Salary":"Lower Salary (K USD)","Upper Salary":"Upper Salary (K USD)", "Rating":"Company Rating"})
...
with row0_1:
    st.title('DataProAn - Data Professionals Salaries Analyzer')
with row0_2:
    st.text("")
...
start_salary, end_salary = st.sidebar.select_slider('Select the average salary range you want to include', boundary_salary,value=[min(boundary_salary),max(boundary_salary)])
...
def plot_Number_Jobs_per_Job_Title(attr): #total #against, #conceived
    ...
    ax.set(xlabel = "Job Title", ylabel ="Number" )
...
### CORRELATION ###
label_attr_dict_correlation={'Average Salary':'Average Salary (K USD)','Upper Salary':'Upper Salary (K USD)', 'Lower Salary':'Lower Salary (K USD)','Company Age':'Company Age', 'Company Name':'Company Name', 'Company Rating':'Company Rating'}
...
with row11_1:
    ...
    x_axis_aspect1 = st.selectbox ("Which attribute do you want on the x-axis ?", ["Company Age", "Company Rating"])
with row11_2:
